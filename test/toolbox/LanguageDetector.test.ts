...
describe('LanguageDetector', () => {
  ...
  describe('detectScript', () => {
    ...
    it('should not ignore punctuation when specified', () => {
      ...
      // So it should return 'la' (fallback)
      expect(detector.detectScript('αβγ.', false)).toBe('la')
    })
  })

  describe('detectLang', () => {
    it('should detect language based on character range', () => {
      expect(detector.detectLang('Hello')).toBe('la')
      expect(detector.detectLang('العربية')).toBe('ar')
      expect(detector.detectLang('עברית')).toBe('he')
      expect(detector.detectLang('Ελληνικά')).toBe('el')
    })

    it('should correctly handle Hebrew strings with various quotation marks', () => {
      const cases = ['ב”הסתעפות“', 'ב’הסתעפות‘', '[הוורידים]']
      for (const text of cases) {
        expect(detector.detectLang(text)).toBe('he')
        expect(detector.detectScript(text)).toBe('he')
        expect(detector.detectTextDirection(text)).toBe('rtl')
      }
    })

    it('should use default language when no match is found', () => {
      const arDetector = new LanguageDetector('ar')
      ...
      expect(arDetector.detectLang('😊')).toBe('ar')
    })

    it('should handle mixed content by picking the majority', () => {
      // Majority Latin
      expect(detector.detectLang('Hello ابج')).toBe('la')
      // Majority Arabic
      expect(detector.detectLang('العربية abc')).toBe('ar')
    })
    ...
    it('should handle default language preference', () => {
      ...
      expect(arDetector.detectLang('12345')).toBe('ar')
    })
  })
})
